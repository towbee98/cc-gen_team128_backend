...
const { validateDesignBody } = require("../validations/design.validation");

const uploadDesign = catchAsync(async (req, res, next) => {
	// validate design body
	const { error } = validateDesignBody(req.body);
	...
	if (!newDesign)
		return next(new AppError("Error occured while uploading the design", 400));
	res.status(201).json({
		status: "success",
...
const fetchDesign = catchAsync(async (req, res, next) => {
	...
	if (!design)
		return next(new AppError("Design not found or does not exist", 404));
	res.status(200).json({
		status: "success",
